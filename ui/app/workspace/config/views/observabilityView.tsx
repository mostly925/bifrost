...
export default function ObservabilityView() {
	...
	const handleSave = useCallback(async () => {
		if (!bifrostConfig) {
			toast.error(t("observability.toasts.saveFailedConfigNotLoaded"));
			return;
		}
		try {
			await updateCoreConfig({ ...bifrostConfig, client_config: localConfig }).unwrap();
			toast.success(t("observability.toasts.updated"));
		} catch (error) {
					...
					<div className="space-y-2 rounded-sm border p-4">
						<div className="space-y-0.5">
							<label htmlFor="prometheus-labels" className="text-sm font-medium">
								{t("observability.prometheusLabelsTitle")}
							</label>
							<p className="text-muted-foreground text-sm">{t("observability.prometheusLabelsDescription")}</p>
						</div>
						<Textarea
