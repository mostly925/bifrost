		...
		<DropdownMenu>
			<DropdownMenuTrigger asChild>
				<Button
					...
					aria-label={t("addProvider.triggerAria")}
					disabled={disabled}
				>
					<PlusIcon className="h-4 w-4" />
					{variant === "empty" ? <span>{t("addProvider.triggerEmpty")}</span> : <div className="text-xs">{t("addProvider.trigger")}</div>}
				</Button>
			</DropdownMenuTrigger>
			<DropdownMenuContent
				align="start"
			...
			>
				...
				<DropdownMenuItem data-testid="add-provider-option-custom" onSelect={onAddCustomProvider}>
					...
					<span>{t("addProvider.customProvider")}</span>
				</DropdownMenuItem>
			</DropdownMenuContent>
		</DropdownMenu>
	);
