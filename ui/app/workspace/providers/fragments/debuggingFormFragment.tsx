...
export function DebuggingFormFragment({ provider }: DebuggingFormFragmentProps) {
	...
	const onSubmit = (data: DebuggingFormSchema) => {
		...
		updateProvider(updatedProvider)
			.unwrap()
			.then(() => {
				toast.success(t("debugging.toasts.updated"));
				form.reset(data);
			})
			.catch((err) => {
				toast.error(t("debugging.toasts.updateFailed"), {
								...
								<div className="flex items-center justify-between space-x-2">
									<div className="space-y-0.5">
										<div className="flex items-center gap-1.5">
											<FormLabel>{t("debugging.rawRequest.label")}</FormLabel>
											<TooltipProvider>
												<Tooltip>
													...
													<TooltipContent>
														{t("debugging.overrideNote")} <code>x-bf-send-back-raw-request: {String(!sendBackRawRequest)}</code>
													</TooltipContent>
												</Tooltip>
											</TooltipProvider>
										</div>
										<p className="text-muted-foreground text-xs">{t("debugging.rawRequest.description")}</p>
									</div>
									<FormControl>
										<Switch
											size="md"
						...
						render={({ field }) => (
							<FormItem>
								<div className="flex items-center justify-between space-x-2">
									<div className="space-y-0.5">
										<div className="flex items-center gap-1.5">
											<FormLabel>{t("debugging.rawResponse.label")}</FormLabel>
											<TooltipProvider>
												<Tooltip>
													...
													<TooltipContent>
														{t("debugging.overrideNote")} <code>x-bf-send-back-raw-response: {String(!sendBackRawResponse)}</code>
													</TooltipContent>
												</Tooltip>
											</TooltipProvider>
										</div>
										<p className="text-muted-foreground text-xs">{t("debugging.rawResponse.description")}</p>
									</div>
									<FormControl>
										<Switch
											size="md"
						...
						render={({ field }) => (
							<FormItem>
								<div className="flex items-center justify-between space-x-2">
									<div className="space-y-0.5">
										<div className="flex items-center gap-1.5">
											<FormLabel>{t("debugging.storeRaw.label")}</FormLabel>
											<TooltipProvider>
												<Tooltip>
													...
													<TooltipContent>
														{t("debugging.overrideNote")} <code>x-bf-store-raw-request-response: {String(!storeRawRequestResponse)}</code>
													</TooltipContent>
												</Tooltip>
											</TooltipProvider>
										</div>
										<p className="text-muted-foreground text-xs">{t("debugging.storeRaw.description")}</p>
									</div>
									<FormControl>
										<Switch
