		...
		<ChartErrorBoundary resetKey={`${data?.buckets?.length ?? 0}-${totalCachedRead}-${totalPromptTokens}`}>
			<div className="grid h-full grid-rows-[104px_auto] items-start overflow-hidden pt-8">
				<div ref={ref} className="relative h-full w-full grow">
					{!hasData && <div className="text-muted-foreground flex h-full items-center justify-center text-sm">{t("chart.noData")}</div>}
					...
					<div>
						<div className="flex shrink-0 flex-col items-center pt-1 leading-none">
							...
							<div className="mt-1 flex items-center gap-1 text-[11px] text-zinc-400">
								<span>{t("meter.externalCacheCaption")}</span>
								<Tooltip>
									<TooltipTrigger asChild>
										<button
											...
											aria-label={t("meter.externalCacheInfoAria")}
										>
											<Info className="h-3 w-3" />
										</button>
									</TooltipTrigger>
									<TooltipContent side="top">{t("meter.externalCacheInfoTip")}</TooltipContent>
								</Tooltip>
							</div>
						</div>
						<div className="flex shrink-0 flex-wrap items-center justify-center gap-x-4 gap-y-1 pt-2 text-[11px] leading-none">
							<span className="flex items-center gap-1.5">
								...
								<span className="text-primary">{t("meter.cached", { value: formatCompactNumber(totalCachedRead) })}</span>
							</span>
							<span className="flex items-center gap-1.5">
								<span className="h-2 w-2 rounded-full" style={{ backgroundColor: METER_COLORS.input }} />
								<span className="text-muted-foreground">{t("meter.input", { value: formatCompactNumber(totalPromptTokens) })}</span>
							</span>
						</div>
					</div>
				)}
