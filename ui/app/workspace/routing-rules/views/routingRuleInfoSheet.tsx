...
function CopyButton({ value, label, testId }: { value: string; label?: string; testId: string }) {
	...
	const labelValue = label ?? t("info.valueLabel");
		...
		<Tooltip>
			<TooltipTrigger asChild>
				<Button
					...
					onClick={handleCopy}
					aria-label={copied ? t("info.copiedAria", { label: labelValue }) : t("info.copyAria", { label: labelValue })}
					data-testid={testId}
				>
					{copied ? <Check className="h-3.5 w-3.5 text-green-500" /> : <Copy className="h-3.5 w-3.5" />}
				</Button>
			</TooltipTrigger>
			<TooltipContent>{copied ? t("info.copied") : t("info.copyValue", { label: labelValue })}</TooltipContent>
		</Tooltip>
	);
}
				...
				<Badge variant="outline" className="shrink-0 font-medium">
					{isHeader && keyName ? (
						<span className="flex items-center gap-1">
							<span className="text-muted-foreground font-normal">{t("info.header")}</span>
							<span className="font-mono">{keyName}</span>
						</span>
					) : isParam && keyName ? (
						<span className="flex items-center gap-1">
							<span className="text-muted-foreground font-normal">{t("info.param")}</span>
							<span className="font-mono">{keyName}</span>
						</span>
					) : (
						fieldLabel
...
function TargetCard({ target, total }: { target: RoutingRule["targets"][0]; index: number; total: number }) {
	const { t } = useTranslation("routingRules");
	const providerLabel = target.provider ? getProviderLabel(target.provider) : t("info.incomingProvider");
				...
				<div className="flex items-center gap-2.5">
					...
					<div className="flex flex-col">
						...
						{target.model ? (
							...
							<span className="text-muted-foreground text-xs">{t("info.incomingModel")}</span>
						)}
					</div>
				</div>
				<Tooltip>
					...
					<TooltipContent>{t("info.weightTooltip", { weight: target.weight })}</TooltipContent>
				</Tooltip>
			</div>
			{target.key_id && (
				<div className="bg-muted/50 flex items-center gap-1.5 rounded-md px-2 py-1">
					...
					<span className="text-muted-foreground text-xs">{t("info.pinnedKey")}</span>
					<code className="truncate font-mono text-xs">{target.key_id}</code>
					<CopyButton value={target.key_id} label={t("info.keyIdLabel")} testId="routing-rule-copy-key-id-btn" />
				</div>
			)}
		</div>
	);
	...
	return (
		<div className="flex flex-wrap items-center gap-y-2">
			{fallbacks.map((fb, i) => {
				...
				const provider = parts[0] || t("info.incomingProvider");
				const model = parts.length > 1 ? parts.slice(1).join("/") : t("info.incomingModel");
					...
					<>
						<SheetHeader className="flex flex-row items-start justify-between gap-1 p-0">
							<div className="flex flex-col items-start gap-1">
								<div className="flex w-full flex-wrap items-center gap-2">
									...
									<Badge variant={rule.enabled ? "default" : "secondary"}>{rule.enabled ? t("info.enabled") : t("info.disabled")}</Badge>
									{rule.chain_rule && (
										<Tooltip>
											<TooltipTrigger asChild>
											...
											<TooltipContent className="max-w-64">{t("info.chainRuleTooltip")}</TooltipContent>
										</Tooltip>
									)}
								</div>
						...
						<div className="-mx-8 space-y-6 overflow-y-auto px-8 pb-8">
							...
							<div className="space-y-3">
								<h3 className="text-sm font-semibold">{t("info.overview")}</h3>
								<div className="grid gap-3">
									<div className="grid grid-cols-3 items-center gap-4">
										<span className="text-muted-foreground text-sm">{t("info.scope")}</span>
									...
									<div className="grid grid-cols-3 items-center gap-4">
										<span className="text-muted-foreground text-sm">{t("info.priority")}</span>
							...
							{/* Conditions */}
							<div className="space-y-3">
								<h3 className="text-sm font-semibold">{t("info.conditions")}</h3>
								{hasQuery ? (
									<ConditionGroup group={rule.query!} />
								) : hasCel ? (
									<p className="text-muted-foreground text-sm">{t("info.definedAsCel")}</p>
								) : (
									<p className="text-muted-foreground text-sm">{t("info.matchesAll")}</p>
								)}

								{/* CEL expression */}
								<div className="space-y-1.5">
									<div className="flex items-center justify-between">
										<span className="text-sm font-semibold">{t("info.celExpression")}</span>
										<CopyButton value={rule.cel_expression} label={t("info.expressionLabel")} testId="routing-rule-copy-expression-btn" />
									</div>
							...
							{/* Targets */}
							<div className="space-y-3">
								<h3 className="text-sm font-semibold">{t("info.targets", { count: targets.length })}</h3>
								{targets.length > 0 ? (
									...
									<p className="text-muted-foreground text-sm">{t("info.noTargets")}</p>
								)}
							</div>
							...
							{/* Fallback Chain */}
							<div className="space-y-3">
								<h3 className="text-sm font-semibold">{t("info.fallbackChain")}</h3>
								{fallbacks.length > 0 ? (
									<FallbackChain fallbacks={fallbacks} />
								) : (
									<p className="text-muted-foreground text-sm">{t("info.noFallbacks")}</p>
								)}
							</div>
							...
							<div className="grid grid-cols-2 gap-4">
								<div>
									<p className="text-muted-foreground mb-1 text-xs font-medium tracking-wider uppercase">{t("info.created")}</p>
								...
								<div>
									<p className="text-muted-foreground mb-1 text-xs font-medium tracking-wider uppercase">{t("info.lastUpdated")}</p>
									<span className="text-sm">
