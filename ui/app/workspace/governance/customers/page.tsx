...
export default function GovernanceCustomersPage() {
	...
	useEffect(() => {
		...
		if (teamsError && customersError) {
			toast.error(t("errors.load.governance"));
		} else {
			if (teamsError) toast.error(t("errors.load.teams", { error: getErrorMessage(teamsError) }));
			if (customersError) toast.error(t("errors.load.customers", { error: getErrorMessage(customersError) }));
		}
