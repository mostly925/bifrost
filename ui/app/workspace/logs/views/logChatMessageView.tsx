...
export function LogChatFileBlockView({ block, className }: { block: ContentBlock; className?: string }) {
	...
	const title = file.filename || file.file_id || t("chat.attachedFile");
...
function ContentBlockView({ block }: { block: ContentBlock; index: number }) {
	...
	if (block.image_url) {
		...
		if (src) {
			return <img src={src} alt={t("chat.attachedImage")} className="max-w-full rounded border" />;
		}
	}
		...
		<div className="flex w-full flex-col gap-2">
			...
			<div className="flex items-center gap-2">
				...
				{message.tool_call_id && (
					<span className="text-muted-foreground text-xs">{t("chat.toolCallId", { id: message.tool_call_id })}</span>
				)}
			</div>
			...
			{message.reasoning && (
				<>
					{isJson(message.reasoning) ? (
						...
						<CollapsibleBox title={t("chat.reasoning")} onCopy={() => message.reasoning || ""} collapsedHeight={100}>
			...
			{message.refusal && (
				<>
					{isJson(message.refusal) ? (
						...
						<CollapsibleBox title={t("chat.refusal")} onCopy={() => message.refusal || ""} collapsedHeight={100}>
					...
					{typeof message.content === "string" ? (
						<>
							{isJson(message.content) ? (
								...
								<CollapsibleBox title={t("chat.content")} onCopy={() => (message.content as string) || ""} collapsedHeight={100}>
									<div className="custom-scrollbar max-h-[400px] overflow-y-auto px-6 py-2 font-mono text-xs break-words whitespace-pre-wrap">
										{message.content}
									</div>
								</CollapsibleBox>
				...
				<>
					{message.tool_calls.map((toolCall, index) => {
						...
						return (
							<CollapsibleBox
								key={index}
								title={t("chat.toolCall", { name: toolCall.function?.name || `#${index + 1}` })}
								onCopy={() => jsonContent}
								collapsedHeight={100}
							>
								<CodeEditor
			...
			{message.annotations && message.annotations.length > 0 && (
				<CollapsibleBox title={t("chat.annotations")} onCopy={() => JSON.stringify(message.annotations, null, 2)} collapsedHeight={100}>
					<CodeEditor
						className="z-0 w-full"
						shouldAdjustInitialHeight={true}
						maxHeight={400}
			...
			{/* Handle audio output */}
			{message.audio && (
				<CollapsibleBox title={t("chat.audioOutput")} collapsedHeight={150}>
					<div className="space-y-4 px-6 py-4">
						{message.audio.transcript && (
							<div className="space-y-2">
								<div className="text-muted-foreground text-xs font-medium">{t("chat.transcript")}</div>
								<div className="font-mono text-xs break-words whitespace-pre-wrap">{message.audio.transcript}</div>
							</div>
						)}
						{message.audio.data && (
							<div className="space-y-2">
								<div className="text-muted-foreground text-xs font-medium">{t("chat.audio")}</div>
