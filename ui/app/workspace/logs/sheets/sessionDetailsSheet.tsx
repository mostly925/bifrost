...
export function SessionDetailsSheet({
	...
	const summaryCards: SummaryCard[] = useMemo(
		() => [
			{
				label: t("session.cards.logs"),
				value: (sessionSummary?.count || 0).toLocaleString(),
				helper:
					sessionSummary && sessionLogs.length < sessionSummary.count
						? t("session.cards.loaded", { count: sessionLogs.length.toLocaleString() })
						: undefined,
			},
			{
				label: t("session.cards.totalCost"),
				value: `$${(sessionSummary?.total_cost || 0).toFixed(4)}`,
			},
			{
				label: t("session.cards.totalTokens"),
				value: (sessionSummary?.total_tokens || 0).toLocaleString(),
			},
			{
				label: t("session.cards.started"),
				value: sessionSummary?.started_at ? format(new Date(sessionSummary.started_at), "MMM d, yyyy hh:mm:ss aa") : t("common.na"),
				size: "sm",
			},
			{
				label: t("session.cards.latestUpdate"),
				value: sessionSummary?.latest_at ? format(new Date(sessionSummary.latest_at), "MMM d, yyyy hh:mm:ss aa") : t("common.na"),
				size: "sm",
			},
			{
				label: t("session.cards.duration"),
	...
	const loadSessionPage = useCallback(
		async (offset: number, reset = false) => {
			...
			try {
				...
				if (result.error) {
					toast.error(t("session.loadFailed"), {
		...
		<Sheet open={open} onOpenChange={onOpenChange}>
			<SheetContent className="flex w-full flex-col gap-4 overflow-x-hidden p-8 sm:max-w-[60%]">
				<div className="flex items-center justify-between gap-4">
					<div>
						<div className="text-lg font-medium">{t("session.title")}</div>
						{sessionId && onFilterByParentRequestId ? (
							<Tooltip>
								...
								<TooltipContent sideOffset={6}>{t("session.filterSessionTooltip")}</TooltipContent>
							</Tooltip>
						) : (
					...
					<div className="flex items-center gap-3">
						...
						>
							...
							{sortOrder === "asc" ? t("session.earliestFirst") : t("session.latestFirst")}
						</Button>
					</div>
				</div>
					...
					<Table containerClassName="h-full overflow-auto">
						<TableHeader className="sticky top-0 z-10 bg-[#f9f9f9] dark:bg-[#27272a]">
							<TableRow>
								...
								<TableHead>{t("session.columns.time")}</TableHead>
								<TableHead>{t("session.columns.type")}</TableHead>
								<TableHead>{t("session.columns.message")}</TableHead>
								<TableHead>{t("session.columns.provider")}</TableHead>
								<TableHead>{t("session.columns.model")}</TableHead>
							</TableRow>
						</TableHeader>
						<TableBody>
							{loadingSession && sessionLogs.length === 0 ? (
								...
								sessionLogs.map((log) => (
									...
									>
										...
										<TableCell className="max-w-[140px] truncate font-mono text-xs">{log.model || t("common.na")}</TableCell>
									</TableRow>
								))
							) : (
								<TableRow>
