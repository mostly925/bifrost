...
export function PrometheusFormFragment({
	...
	const renderActions = (tabKey: "pull" | "push", tabDirty: boolean, onResetTab: () => void) => {
		...
		const otherTabLabel = tabKey === "pull" ? t("prometheus.tabs.push") : t("prometheus.tabs.pull");
		const saveDisabled = !hasPrometheusAccess || !tabDirty || formIsInvalid;
		let tooltipMsg = "";
		if (!tabDirty) {
			tooltipMsg = t("prometheus.tooltips.noChangesInTab");
		} else if (formIsInvalid && otherTabHasErrors && !thisTabHasErrors) {
			tooltipMsg = t("prometheus.tooltips.fixOtherTab", { tab: otherTabLabel });
		} else if (formIsInvalid) {
			tooltipMsg = t("prometheus.tooltips.fixValidationErrors");
		}
				...
				<Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "pull" | "push")}>
					...
					<TabsContent value="pull" className="mt-2 space-y-4">
						<div className="flex items-center justify-between gap-4">
							<div className="flex flex-col gap-1">
								<h3 className="text-sm font-medium">{t("prometheus.pull.title")}</h3>
								<p className="text-muted-foreground text-xs">{t("prometheus.pull.description")}</p>
							</div>
							<FormField
						...
						<div className="bg-muted/50 rounded-md p-4">
							<div className="flex items-center justify-between">
								<div className="flex flex-col gap-1">
									<span className="text-sm font-medium">{t("prometheus.pull.endpoint")}</span>
							...
							<p className="text-muted-foreground mt-2 text-xs">{t("prometheus.pull.scrapeHint")}</p>
						</div>
					...
					<TabsContent value="push" className="mt-2 space-y-4">
						<div className="flex items-center justify-between gap-4">
							<div className="flex flex-col gap-1">
								<h3 className="flex flex-row items-center gap-2 text-sm font-medium">
									{t("prometheus.push.title")} <Badge variant="secondary">BETA</Badge>
								</h3>
								<p className="text-muted-foreground text-xs">{t("prometheus.push.description")}</p>
							</div>
							<FormField
								control={form.control}
						...
						<Alert variant="info">
							...
							<AlertDescription className="text-xs">{t("prometheus.push.multiNodeWarning")}</AlertDescription>
						</Alert>
						...
						<div className="space-y-4">
							<FormField
								...
								render={({ field }) => (
									<FormItem className="w-full">
										<FormLabel>{t("prometheus.push.gatewayUrl")}</FormLabel>
										<FormControl>
											<SecretVarInput
										...
										<FormDescription>{t("prometheus.push.gatewayUrlDescription")}</FormDescription>
										<FormMessage />
									</FormItem>
								)}
							/>
							...
							<div className="grid grid-cols-2 gap-4">
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>{t("prometheus.push.jobName")}</FormLabel>
											<FormControl>
												<Input placeholder="bifrost" disabled={!hasPrometheusAccess} data-testid="prometheus-job-name" {...field} />
											</FormControl>
											<FormDescription>{t("prometheus.push.jobNameDescription")}</FormDescription>
											<FormMessage />
										</FormItem>
									)}
								/>
								...
								<FormField
									...
									render={({ field }) => (
										<FormItem>
											<FormLabel>{t("prometheus.push.pushInterval")}</FormLabel>
											<FormControl>
												<Input
													type="number"
													min={1}
											...
											<FormDescription>{t("prometheus.push.pushIntervalDescription")}</FormDescription>
											<FormMessage />
										</FormItem>
									)}
								/>
							...
							<FormField
								...
								render={({ field }) => (
									<FormItem>
										<FormLabel className="flex items-center gap-2">
											...
											<TooltipProvider>
												<Tooltip>
													...
													<TooltipContent>
														<p className="max-w-xs text-xs">{t("prometheus.push.instanceIdTooltip")}</p>
													</TooltipContent>
												</Tooltip>
											</TooltipProvider>
										</FormLabel>
										<FormControl>
											<Input
												placeholder={t("prometheus.push.instanceIdPlaceholder")}
												disabled={!hasPrometheusAccess}
							...
							<div className="space-y-4 border-t pt-4">
								{!showBasicAuth ? (
									...
									>
										...
										{t("prometheus.push.addBasicAuth")}
									</Button>
								) : (
									<>
										<div className="flex items-center justify-between">
											<span className="text-sm font-medium">{t("prometheus.push.basicAuth")}</span>
											<Button
												type="button"
												variant="ghost"
												size="sm"
												...
												aria-label={t("prometheus.push.removeBasicAuthAria")}
											>
										...
										<div className="border-muted grid grid-cols-2 gap-4">
											<FormField
												...
												render={({ field }) => (
													<FormItem>
														<FormLabel>{t("prometheus.push.username")}</FormLabel>
														<FormControl>
															<SecretVarInput
																placeholder={t("prometheus.push.usernamePlaceholder")}
																disabled={!hasPrometheusAccess}
											...
											<FormField
												...
												render={({ field }) => (
													<FormItem>
														<FormLabel>{t("prometheus.push.password")}</FormLabel>
														<FormControl>
															<SecretVarInput
																type="password"
																placeholder={t("prometheus.push.passwordPlaceholder")}
																disabled={!hasPrometheusAccess}
																hideValueWhenEnv
																redactNonEnvValue
