...
import { localizedBudgetResetDurationOptions } from "@/lib/constants/governance";
...
export default function MultiBudgetLines({
	...
	// Labels resolve through the shared namespace so period names follow the UI language.
	const resolvedOptions = options ?? localizedBudgetResetDurationOptions(t);
	...
	return (
		<div className="space-y-3" data-testid={testId}>
			<div className="flex items-center justify-between">
				<Label className="text-sm font-medium">{label ?? t("budgetLines.label")}</Label>
				<div className="flex items-center gap-2">
					...
					<Button data-testid={`${testId}-add-btn`} variant="outline" size="sm" type="button" onClick={addLine}>
						...
						{t("budgetLines.addBudget")}
					</Button>
				</div>
			</div>

			{lines.length === 0 && (
				<div className="text-muted-foreground rounded-md border border-dashed p-3 text-center text-sm">{t("budgetLines.empty")}</div>
			)}
					...
					<div key={index} className="space-y-1" data-testid={`${testId}-line-${index}`}>
						<div className="flex items-end gap-2">
							...
							<Button
								...
								aria-label={t("budgetLines.removeAria", { index: index + 1 })}
								variant="ghost"
								size="icon"
								type="button"
						...
						{isDuplicate && <p className="text-destructive pl-0.5 text-xs">{t("budgetLines.duplicate")}</p>}
					</div>
				);
			})}
		</div>
