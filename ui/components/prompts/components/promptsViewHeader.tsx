...
export default function PromptsViewHeader() {
	...
	const handleSaveSession = useCallback(async () => {
		...
		try {
			...
			toast.error(t("toasts.sessionSaveFailed"), { description: getErrorMessage(err) });
		}
	...
	const handleRenameSession = useCallback(
		async (sessionId: number, name: string) => {
			...
			try {
				...
				toast.error(t("toasts.sessionRenameFailed"), { description: getErrorMessage(err) });
			}
		},
		...
		<div className="flex items-center justify-between border-b px-4 py-3">
			<div className="flex min-w-0 items-center gap-2">
				<h3 className="truncate font-semibold">
					{selectedPrompt?.name || t("header.playground")}
				...
				{hasVersionChanges && versions.length > 0 && <Badge variant="outline">{t("header.unpublishedChanges")}</Badge>}
			</div>
			<div className="flex shrink-0 items-center gap-4">
				...
				<SplitButton
					...
					dropdownContent={{
						...
						children: (
							<Command>
								<CommandInput placeholder={t("header.searchSessions")} data-testid="header-sessions-search" />
								<CommandList>
									<CommandEmpty>{t("header.noSessions")}</CommandEmpty>
									<CommandGroup>
				...
				<SplitButton
					...
					dropdownContent={{
						...
						children: (
							<>
								<DropdownMenuLabel>{t("header.versions")}</DropdownMenuLabel>
								<DropdownMenuSeparator />
								{versions.length === 0 ? (
									<div className="text-muted-foreground px-2 py-3 text-center text-sm">{t("header.noVersions")}</div>
								) : (
										...
										>
											<div className="flex min-w-0 flex-col">
												<span className="truncate text-sm">
													...
													{version.is_latest && <span className="text-primary ml-1.5 text-xs">{t("header.latest")}</span>}
												</span>
												<span className="text-muted-foreground truncate text-xs">
													{version.commit_message || t("header.noCommitMessage")}
												</span>
	...
	return (
		...
		>
			...
			<div className="flex shrink-0 items-center gap-1">
				<button
					type="button"
					aria-label={t("header.renameSessionAria")}
