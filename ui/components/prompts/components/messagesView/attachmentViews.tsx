	...
	return (
		<div className="group/att bg-muted/50 relative flex items-center gap-1.5 rounded-sm border px-2 py-1 text-xs">
			{isImage && attachment.image_url?.url ? (
				<>
					<img src={attachment.image_url.url} alt={t("messages.attachmentAlt")} className="h-8 w-8 rounded object-cover" />
					<span className="text-muted-foreground max-w-[100px] truncate">{t("messages.imageFallback")}</span>
				</>
			) : isAudio ? (
				<>
					<Mic className="text-muted-foreground size-3" />
					<span className="text-muted-foreground max-w-[100px] truncate">
						{attachment.input_audio?.format?.toUpperCase() || t("messages.audioFallback")}
					</span>
				</>
			) : (
				<>
					<FileIcon className="text-muted-foreground size-3" />
					<span className="text-muted-foreground max-w-[120px] truncate">{attachment.file?.filename || t("messages.fileFallback")}</span>
				</>
			)}
			<button
				onClick={onRemove}
					...
					return (
						<div key={i} className="group/att relative max-w-full">
							...
							<img
								...
								alt={t("messages.attachedImageAlt")}
				...
				if (att.type === "file") {
					return (
						...
						>
							...
							<span className="min-w-0 truncate">{att.file?.filename || t("messages.fileFallback")}</span>
