					...
					<div className="flex flex-col p-2 px-3">
						{filteredData.folders.length === 0 && filteredData.rootPrompts.length === 0 ? (
							<div className="text-muted-foreground py-8 text-center text-sm">
								{searchQuery ? t("sidebar.noResults") : t("sidebar.empty")}
							</div>
						) : (
							<>
...
function DroppableFolder({
	...
	return (
		<div ref={ref} className="mb-1 last:mb-0">
			...
			>
				<button className="flex shrink-0 items-center" aria-label={t("sidebar.toggleFolderAria")}>
					{isExpanded ? (
					...
					<DropdownMenu>
						<DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()} className="bg-card absolute top-1/2 right-2 -translate-y-1/2">
							<Button
								...
								aria-label={t("sidebar.folderActionsAria")}
							>
			...
			{isExpanded && (
				<div className="ml-4 border-l pl-2">
					{prompts.length === 0 ? (
						<div className="text-muted-foreground py-2 pl-4 text-xs">{isDragOver ? t("sidebar.dropHere") : t("sidebar.noPrompts")}</div>
					) : (
						prompts.map((prompt) => (
							<DraggablePromptItem
								key={prompt.id}
				...
				<DropdownMenu>
					<DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
						<Button
							...
							aria-label={t("sidebar.promptActionsAria")}
						>
							<MoreHorizontal className="h-4 w-4" />
						</Button>
					</DropdownMenuTrigger>
