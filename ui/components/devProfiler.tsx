...
function GoroutineHealthSection({
	...
	const { t } = useTranslation("devProfiler");
		...
		<div className="p-3">
			...
			<div className="mb-2 flex items-center justify-between">
				<div className="flex items-center gap-2">
					...
					<span className="text-zinc-400">{t("goroutineHealth.title")}</span>
				</div>
				<div className="flex items-center gap-2">
					{goroutineTrend?.isGrowing && (
						<span className="flex items-center gap-1 text-amber-400" title={t("goroutineHealth.growingTitle")}>
			...
			<div className="mb-2 grid grid-cols-4 gap-2 rounded bg-zinc-800/50 p-2">
				<div className="flex flex-col items-center">
					<span className="text-[10px] text-zinc-500">{t("goroutineHealth.total")}</span>
					<span className="font-semibold text-emerald-400">{total_goroutines}</span>
				</div>
				<div className="flex flex-col items-center">
					<span className="text-[10px] text-zinc-500">{t("goroutineHealth.background")}</span>
					<span className="font-semibold text-blue-400">{summary.background}</span>
				</div>
				<div className="flex flex-col items-center">
					<span className="text-[10px] text-zinc-500">{t("goroutineHealth.perRequest")}</span>
					<span className="font-semibold text-amber-400">{summary.per_request}</span>
				</div>
				<div className="flex flex-col items-center">
					<span className="text-[10px] text-zinc-500">{t("goroutineHealth.stuck")}</span>
			...
			{(problemGoroutines.length > 0 || skippedGoroutines.size > 0) && (
				<div className="space-y-1">
					<div className="flex items-center justify-between">
						<span className="text-[10px] text-zinc-500">{t("goroutineHealth.potentialLeaks")}</span>
						{skippedGoroutines.size > 0 && (
							<button
								...
								title={t("goroutineHealth.clearHiddenTitle")}
							>
								<RotateCcw className="h-2.5 w-2.5" />
								{t("goroutineHealth.hiddenCount", { count: skippedGoroutines.size })}
							</button>
						)}
					</div>
							...
							<div key={gid} className="group relative rounded bg-zinc-800">
								...
								>
									...
									<div className="flex items-center gap-2 pl-5 text-[10px]">
										...
										{g.wait_minutes != null && (
											<span className="text-amber-400">{t("goroutineHealth.waitingMinutes", { minutes: g.wait_minutes })}</span>
										)}
									</div>
								</div>
								<button
									...
									title={t("goroutineHealth.hideFileTitle")}
								>
								...
								{expandedGoroutines.has(gid) && (
									<div className="border-t border-zinc-700 bg-zinc-900/50 px-2 py-1.5">
										<div className="mb-1 text-[10px] text-zinc-500">
											{t("goroutineHealth.stateLabel")} <span className="text-zinc-400">{g.state}</span>
											{g.wait_reason && (
												<span className="ml-2">
													{t("goroutineHealth.waitLabel")} <span className="text-amber-400">{g.wait_reason}</span>
												</span>
											)}
										</div>
										<div className="max-h-32 overflow-x-hidden overflow-y-auto">
											...
											{g.stack.length > 10 && (
												<div className="text-[9px] text-zinc-600">{t("goroutineHealth.moreFrames", { count: g.stack.length - 10 })}</div>
											)}
										</div>
									</div>
								)}
					...
					{problemGoroutines.length === 0 && skippedGoroutines.size > 0 && (
						<div className="rounded bg-zinc-800/30 py-2 text-center text-[10px] text-zinc-500">{t("goroutineHealth.allHidden")}</div>
					)}
					{problemGoroutines.length === 0 &&
						...
						(summary.long_waiting > 0 || summary.potentially_stuck > 0) && (
							<div className="rounded bg-zinc-800/30 px-2 py-2 text-center text-[10px] text-zinc-500">
								{summary.long_waiting > 0 && summary.potentially_stuck > 0
									? t("goroutineHealth.longAndStuck", { longWaiting: summary.long_waiting, stuck: summary.potentially_stuck })
									: summary.long_waiting > 0
										? t("goroutineHealth.longWaitingOnly", { longWaiting: summary.long_waiting })
										: t("goroutineHealth.stuckOnly", { stuck: summary.potentially_stuck })}
							</div>
						)}
				</div>
			)}
			...
			{problemGoroutines.length === 0 && summary.long_waiting === 0 && summary.potentially_stuck === 0 && (
				<div className="rounded bg-zinc-800/30 py-2 text-center text-[10px] text-zinc-500">{t("goroutineHealth.noLeaks")}</div>
			)}
		</div>
	);
}
	...
	return (
		<div className="fixed right-4 bottom-4 z-50 w-[420px] overflow-hidden rounded-lg border border-zinc-700 bg-zinc-900 font-mono text-xs text-zinc-100 shadow-2xl">
			...
			<div className="flex items-center justify-between border-b border-zinc-700 bg-zinc-800 px-3 py-2">
				<div className="flex items-center gap-2">
					<span className="font-semibold text-emerald-400">{t("panel.title")}</span>
				...
				<div className="flex items-center gap-1">
					<button
						...
						title={isExpanded ? t("panel.collapseTitle") : t("panel.expandTitle")}
					>
					...
					<button
						onClick={handleToggleVisible}
						className="rounded p-1 transition-colors hover:bg-zinc-700"
						title={t("panel.minimizeTitle")}
					>
						<ChevronDown className="h-4 w-4" />
					</button>
					<button onClick={handleDismiss} className="rounded p-1 transition-colors hover:bg-zinc-700" title={t("panel.dismissTitle")}>
						<X className="h-4 w-4" />
					</button>
				</div>
			</div>

			{Boolean(error) && <div className="border-b border-zinc-700 bg-red-900/30 px-3 py-2 text-red-300">{t("panel.loadError")}</div>}
				...
				<div className="custom-scrollbar max-h-[70vh] overflow-x-hidden overflow-y-auto">
					...
					<div className="border-b border-zinc-700 p-3">
						<div className="mb-2 flex items-center gap-2">
							...
							<span className="text-zinc-400">{t("charts.cpuTitle")}</span>
						</div>
						<div className="h-24">
					...
					<div className="border-b border-zinc-700 p-3">
						<div className="mb-2 flex items-center gap-2">
							...
							<span className="text-zinc-400">{t("charts.memoryTitle")}</span>
						</div>
						<div className="h-24">
							<ResponsiveContainer width="100%" height="100%">
					...
					<div className="border-b border-zinc-700 p-3">
						<div className="mb-2 flex items-center gap-2">
							...
							<span className="text-zinc-400">{t("topAllocations.title")}</span>
						</div>
						<div className="space-y-1">
							{(data.top_allocations ?? []).map((alloc, i) => (
								<div key={i} className="flex items-center justify-between rounded bg-zinc-800 px-2 py-1">
									...
									<div className="flex flex-col items-end">
										...
										<span className="text-[10px] text-zinc-500">
											{t("topAllocations.allocCount", { num: alloc.count.toLocaleString() })}
										</span>
									</div>
								</div>
							))}
					...
					<div className="border-t border-zinc-700 bg-zinc-800 px-3 py-2 text-[10px] text-zinc-500">
						{t("footer.runtime", {
							cpus: data.runtime.num_cpu,
							gomaxprocs: data.runtime.gomaxprocs,
							gcs: data.runtime.num_gc,
							objects: data.memory.heap_objects.toLocaleString(),
						})}
					</div>
				</div>
			)}
			...
			{!isExpanded && data && (
				<div className="flex items-center justify-between bg-zinc-800/50 px-3 py-2">
					<span className="text-orange-400">{t("collapsed.cpu", { value: `${data.cpu.usage_percent.toFixed(1)}%` })}</span>
					<span className="text-zinc-400">{t("collapsed.heap", { value: formatBytes(data.memory.heap_inuse) })}</span>
					<span className="text-zinc-400">{t("collapsed.goroutines", { value: data.runtime.num_goroutine })}</span>
				</div>
			)}
		</div>
	);
