			...
			<div className="flex h-11 items-center justify-between border-b pr-2 pl-5">
				...
				<div className="flex items-center gap-1">
					...
					<Button
						...
						aria-label={t("sidebar.hideFilters")}
					>
			...
			{showAddCustom && (
				...
				>
					...
					<span className="truncate">
						<Trans i18nKey="list.useValue" ns="filters" values={{ value: trimmed }} components={{ 1: <span className="font-medium" /> }} />
					</span>
				</button>
			)}
		</>
...
function StatusFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection title={t("sections.status")} defaultOpen={defaultOpen || hasActive} testId="status-filter-toggle">
			{Statuses.map((status) => (
				<CheckboxFilterItem
					key={status}
	...
	return (
		...
		>
			<SearchableCheckboxList
				...
				placeholder={t("placeholders.stopReason")}
				items={items}
				allowCustom
...
function TypeFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection title={t("sections.type")} defaultOpen={defaultOpen || hasActive} testId="type-filter-toggle">
...
function SelectedKeysFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		...
		>
			<SearchableCheckboxList
				inputRef={searchInputRef}
				placeholder={t("placeholders.keys")}
...
function RoutingEnginesFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection
			title={t("sections.routingEngines")}
			defaultOpen={defaultOpen || hasActive}
			loading={isLoading}
			onOpenChange={setOpened}
		...
		>
			<SearchableCheckboxList
				inputRef={searchInputRef}
				placeholder={t("placeholders.engines")}
...
function SessionFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection title={t("sections.session")} defaultOpen={defaultOpen || hasActive} testId="session-filter-toggle">
			<div className="relative">
				<Search className="text-muted-foreground pointer-events-none absolute top-1/2 left-2.5 size-3.5 -translate-y-1/2" />
				<Input
					value={filters.parent_request_id || ""}
					onChange={(e) => onFiltersChange({ ...filters, parent_request_id: e.target.value })}
					placeholder={t("placeholders.parentRequestId")}
...
function BusinessUnitFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection
			title={t("sections.businessUnits")}
			defaultOpen={defaultOpen || hasActive}
			loading={isLoading}
			onOpenChange={setOpened}
		...
		>
			<SearchableCheckboxList
				inputRef={searchInputRef}
				placeholder={t("placeholders.businessUnit")}
...
function CostFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		<FilterSection title={t("sections.cost")} defaultOpen={defaultOpen || hasActive} testId="cost-filter-toggle">
			<CheckboxFilterItem
				label={t("cost.showMissingOnly")}
...
function LocalCachingFilter({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	const localCachingOptions: { key: string; label: string }[] = [
		{ key: "direct", label: t("localCaching.direct") },
		{ key: "semantic", label: t("localCaching.semantic") },
	];
	return (
		<FilterSection title={t("sections.localCaching")} defaultOpen={defaultOpen || hasActive} testId="local-caching-filter-toggle">
...
function MetadataFilters({ filters, onFiltersChange, defaultOpen }: FilterComponentProps) {
	...
	return (
		...
		>
			{isEmpty ? (
				<div className="text-muted-foreground px-3 py-2 text-xs">{t("metadata.noKeys")}</div>
			) : (
				<>
					<div className="relative border-b">
						{isFetching ? (
						...
						<Input
							...
							placeholder={t("placeholders.metadata")}
							className="h-8 border-0 pl-8 text-xs"
